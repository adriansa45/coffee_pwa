...
export function ShopInfo({
    ...
    const categories = [
        { label: "Café", value: categoryRatings?.avgCoffee || 0, icon: Coffee, color: "text-primary", bg: "bg-primary/10" },
        { label: "Comida", value: categoryRatings?.avgFood || 0, icon: Utensils, color: "text-primary", bg: "bg-primary/10" },
        { label: "Lugar", value: categoryRatings?.avgPlace || 0, icon: Home, color: "text-primary", bg: "bg-primary/10" },
        { label: "Precio", value: categoryRatings?.avgPrice || 0, icon: DollarSign, color: "text-primary", bg: "bg-primary/10" },
    ];
    ...
    return (
        <div className="space-y-8">
            <div className="space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-muted-foreground">
                    <div className="flex items-center gap-1.5 bg-primary/10 px-2.5 py-1 rounded-lg">
                        ...
                        <span className="text-xs text-primary/70">({reviewCount || 0})</span>
                    </div>

                    <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1.5 bg-primary/10 px-2.5 py-1 rounded-lg text-primary">
            ...
            <div className="grid grid-cols-2 gap-2.5">
                {categories.map((cat) => (
                    <div key={cat.label} className="bg-muted/50 rounded-xl p-2.5 border border-border flex flex-col gap-1.5">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-xs font-bold text-foreground">{cat.value ? Number(cat.value).toFixed(1) : '0.0'}</span>
                        </div>
                    </div>
                ))}
            </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {features.map((feature) => {
                            ...
                            return (
                                <div
                                    key={feature.id}
                                    className="px-4 py-2 rounded-xl text-xs font-bold bg-primary/5 text-primary border border-primary/10 flex items-center gap-2 shadow-sm"
                                >
                                    {IconComponent ? (
                                        <IconComponent size={14} />
                                    ) : (
                                        <div className="w-3.5 h-3.5 flex items-center justify-center bg-foreground/5 rounded-md text-[8px]">
