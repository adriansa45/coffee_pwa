    ...
    if (initialReviews.length === 0) {
        return (
            <div className="text-center py-12 bg-brand-50/30 rounded-3xl border-2 border-dashed border-brand-100">
                <p className="text-sm text-brand-900/40 font-medium">Sé el primero en compartir tu experiencia</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4">
            {initialReviews.map((rev) => (
                <div key={rev.id} className="bg-white p-5 rounded-[28px] border border-brand-50 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm font-bold text-brand-950">{rev.userName}</p>
                                <p className="text-[10px] text-brand-400 font-bold uppercase tracking-wider">Explorador</p>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4 bg-gray-50/50 p-3 rounded-2xl border border-gray-100">
                        ...
                        <div className="flex items-center gap-2">
                            <CircleDollarSign size={14} className="text-brand-600" />
                            <LikertRating rating={rev.priceRating} size="sm" />
                        </div>
                    </div>

                    {rev.comment && (
                        <p className="text-sm text-brand-950/80 leading-relaxed font-medium pl-1">
                            {rev.comment}
                        </p>
                    )}

                    <div className="mt-4 pt-4 border-t border-brand-50 flex justify-between items-center">
